...
public class ExtractFromStDsOutToLoadVztDataShareTransform extends DoFn<String, Row> {
	...
	public ExtractFromStDsOutToLoadVztDataShareTransform(TupleTag validRowsTag, TupleTag invalidRowsTag) {
	...
	private Row buildRowWithSchema(Schema schema, String input) throws ParseException {
		...
		rowBuilder.addValue(ObjectUtils.transformationForEfxBillCnxErrorCode(rowSplitter.next()));
		rowBuilder.addValue(rowSplitter.next());
		rowBuilder.addValue(rowSplitter.next());
		rowBuilder.addValue(rowSplitter.next());
		rowBuilder.addValue(rowSplitter.next());
